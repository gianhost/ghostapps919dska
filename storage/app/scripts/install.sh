...
#START
clear
echo "Welcome on gianhost Cloud Control Panel ;)"
sleep 3s
...
#gianhost START
clear
echo "gianhost remote configuration..."
sleep 3s
...
sudo mkdir /gianhost/
wget $REMOTE/sh/ha/$SERVERCODE/ -O /gianhost/host-add.sh
wget $REMOTE/sh/hd/$SERVERCODE/ -O /gianhost/host-del.sh
wget $REMOTE/sh/aa/$SERVERCODE/ -O /gianhost/alias-add.sh
wget $REMOTE/sh/ad/$SERVERCODE/ -O /gianhost/alias-del.sh
wget $REMOTE/sh/pw/$SERVERCODE/ -O /gianhost/passwd.sh
wget $REMOTE/sh/rt/$SERVERCODE/ -O /gianhost/root.sh
wget $REMOTE/sh/st/ -O /gianhost/status.sh
wget $REMOTE/sh/dy/ -O /gianhost/deploy.sh
wget $REMOTE/sh/sc/ -O /gianhost/ssl.sh
sudo chmod o-r /gianhost

clear
echo "gianhost remote configuration: OK!"
sleep 3s
...
sudo cat > "$WELCOME" <<EOF
   _____ _____          _   _ _    _  ____   _____ _______
...
sudo dos2unix /gianhost/deploy.sh
sudo dos2unix /gianhost/passwd.sh
sudo dos2unix /gianhost/root.sh
sudo dos2unix /gianhost/status.sh
sudo dos2unix /gianhost/ssl.sh
sudo dos2unix /gianhost/host-add.sh
sudo dos2unix /gianhost/host-del.sh
sudo dos2unix /gianhost/alias-add.sh
sudo dos2unix /gianhost/alias-del.sh

shopt -s expand_aliases
alias ll='ls -alF'

DATABASE=/gianhost/dbroot
sudo touch $DATABASE
...
PHPINI72=/etc/php/7.2/fpm/conf.d/gianhost.ini
sudo touch $PHPINI72
...
PHPINI73=/etc/php/7.3/fpm/conf.d/gianhost.ini
sudo touch $PHPINI73
...
PHPINI74=/etc/php/7.4/fpm/conf.d/gianhost.ini
sudo touch $PHPINI74
...
use mysql;
CREATE USER 'gianhost'@'%' IDENTIFIED BY '$DBPASS';
GRANT ALL PRIVILEGES ON *.* TO 'gianhost'@'%' WITH GRANT OPTION;
FLUSH PRIVILEGES;
EOF
...
sudo ssh-keygen -t rsa -C "git@github.com" -f /gianhost/github -q -P ""
...
DBNAME=phpmyadmin
DBUSER=gianhost
/usr/bin/mysql -u gianhost -p$DBPASS <<EOF
...
sudo cat > "$STATUS" <<EOF
    <?php
    echo exec("sh /gianhost/status.sh");
EOF
...
<div class="flex-center position-ref full-height">
    <div class="content">
        <div class="title m-b-md">
            Hey...<br>This is gianhost ;)
        </div>
        <div class="links">
            <a href="https://www.gianhost.com">www.gianhost.com</a>
        </div>
    </div>
</div>
...
clear
echo "gianhost pages creation: OK!"
sleep 3s
echo -e "\n"
...
#END
clear
echo "gianhost installation is finishing. Wait..."
sleep 3s

sudo apt-get upgrade -y
sudo apt-get update

TASK=/etc/cron.d/gianhost.crontab
touch $TASK
cat > "$TASK" <<EOF
...
clear
echo "gianhost installation has been completed... Wait for your data!"
sleep 3s
...
echo "gianhost SSH username: $USER"
echo "gianhost SSH password: $PASS"
echo "gianhost MySql root/gianhost pass: $DBPASS"
echo ""
echo "***********************************************************"
