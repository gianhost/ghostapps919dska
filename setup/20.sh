...
#START
clear
echo "Welcome to GIANHOST INSTALLER v.0.0.1 ;)"
sleep 3s
...
sudo useradd -m -s /bin/bash gianhost
echo "gianhost:$PASS"|chpasswd
sudo usermod -aG sudo gianhost
...
/usr/bin/mysql -u root -p$DBPASS <<EOF
CREATE DATABASE IF NOT EXISTS gianhost;
EOF
clear
sudo rm -rf /var/www/html
sudo mkdir /var/www/html
echo "Downloading gianhost from packagist.org... It may takes some time! Hold on :)"
sleep 1s
composer create-project MarkosTech/ghostapps919dska /var/www/html
cd /var/www/html && sudo unlink .env
cd /var/www/html && sudo cp .env.example .env
sudo rpl -i -w "DB_USERNAME=dbuser" "DB_USERNAME=gianhost" /var/www/html/.env
sudo rpl -i -w "DB_PASSWORD=dbpass" "DB_PASSWORD=$DBPASS" /var/www/html/.env
sudo rpl -i -w "DB_DATABASE=dbname" "DB_DATABASE=gianhost" /var/www/html/.env
...
echo "         |_|      "
echo ""
echo "Visit http://$IP and login into gianhost:"
...
echo "gianhost server username: gianhost"
echo "gianhost server password: $PASS"
echo "gianhost server db root pass: $DBPASS"
echo ""
echo "Enjoy gianhost :)"
echo ""
echo "***********************************************************"
