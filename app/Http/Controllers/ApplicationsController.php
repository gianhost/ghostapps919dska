...
class ApplicationsController extends Controller {
    ...
    public function create(Request $request) {
        ...
        $ssh->setTimeout(360);
        if($base) {
            $response = $ssh->exec('echo '.$server->password.' | sudo -S sudo sh /gianhost/host-add.sh -u '.$user.' -p '.$pass.' -dbp '.$dbpass.' -b '.$base.' -php '.$request->php.' -a '.$appcode.' -r '.$this->url->to('/'));
        } else {
            $response = $ssh->exec('echo '.$server->password.' | sudo -S sudo sh /gianhost/host-add.sh -u '.$user.' -p '.$pass.' -dbp '.$dbpass.' -php '.$request->php.' -a '.$appcode.' -r '.$this->url->to('/'));
        }
        if(strpos($response, '###gianhost###') === false) {
            $request->session()->flash('alert-error', 'There was a problem with server scripts.');
            return redirect('/applications');
        }
        $response = explode('###gianhost###', $response);
    ...
    public function destroy(Request $request) {
        ...
        $ssh->exec('echo '.$application->server->password.' | sudo -S sudo sh /gianhost/host-del.sh -u '.$application->username.' -p '.$application->php);
        $application->delete();
    ...
    public function ssl($appcode) {
        ...
        $ssh->setTimeout(360);
        $response = $ssh->exec('echo '.$application->server->password.' | sudo -S sudo sh /gianhost/ssl.sh -d '.$application->domain.' -c '.$application->username);
        if(strpos($response, '###gianhost###') === false) {
            abort(500);
        }
        $response = explode('###gianhost###', $response);
