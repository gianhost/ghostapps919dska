...
class AliasesController extends Controller {
    ...
    public function create(Request $request) {
        ...
        $ssh->setTimeout(360);
        $response = $ssh->exec('echo '.$application->server->password.' | sudo -S sudo sh /gianhost/alias-add.sh -d '.$request->domain.' -a '.$application->appcode.' -r '.$this->url->to('/'));
    ...
    public function destroy(Request $request) {
        ...
        $ssh->setTimeout(360);
        $ssh->exec('echo '.$alias->application->server->password.' | sudo -S sudo sh /gianhost/alias-del.sh -d '.$alias->domain);
    ...
    public function ssl($aliascode) {
        ...
        $ssh->setTimeout(360);
        $response = $ssh->exec('echo '.$alias->application->server->password.' | sudo -S sudo sh /gianhost/ssl.sh -d '.$alias->domain.' -c '.$alias->domain);
        if(strpos($response, '###gianhost###') === false) {
            abort(500);
        }
        $response = explode('###gianhost###', $response);
