...
class ServersController extends Controller
{
    ...
    public function create(Request $request) {
        ...
        if($request->ip == $request->server('SERVER_ADDR')) {
            $request->session()->flash('alert-error', 'You can\'t install a client server into the same gianhost Server!');
    ...
    public function changeip(Request $request) {
        ...
        if($request->ip == $request->server('SERVER_ADDR')) {
            $request->session()->flash('alert-error', 'You can\'t setup the same gianhost IP!');
            return redirect('/servers');
        }
    ...
    public function reset($servercode) {
        ...
        $ssh->setTimeout(360);
        $response = $ssh->exec('echo '.$server->password.' | sudo -S sudo sh /gianhost/root.sh -p '.$pass);
