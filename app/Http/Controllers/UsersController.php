...
class UsersController extends Controller {
    ...
    public function reset(Request $request) {
        ...
        $ssh->setTimeout(360);
        $response = $ssh->exec('echo '.$application->server->password.' | sudo -S sudo sh /gianhost/passwd.sh -u '.$request->username.' -p '.$pass.' -dbp '.$application->dbpass. ' -dbop '.$application->dbpass);
